  ...
  for(j in c('lin','nonlin','both')){
      ...
      for(i in 1 : length(seasonal.factor)){
          ...
          if(i == 1){
              ...
              seasonal.combs[[i]] <- rbind(replicate(length(seasonal.factor), current.seasonals), as.integer(seasonal.factor))
          }
    ...
    previous.seasonals[[which(c("lin",'nonlin','both')==j)]] <- as.integer(current.seasonals)
    previous.estimates[[which(c("lin",'nonlin','both')==j)]] <- current.estimate

    overall.seasonals[[which(c("lin",'nonlin','both')==j)]] <- as.integer(current.seasonals[length(current.estimate)])
