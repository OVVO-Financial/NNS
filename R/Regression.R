...
NNS.reg = function (x, y,
  ...
  mode = function(x){
    ...
    Dynamic.average.min = mean(mean(y[x <= min.range]), median(y[x <= min.range]),  mode(y[x <= min.range]))
    Dynamic.average.max = mean(mean(y[x >= max.range]), median(y[x >= max.range]), mode(y[x >= max.range]))
