...
#'
#'
#' ## To find average partial derivative of y wrt 1st regressor, for every oberservation of 1st regressor
...
dy.d_<- function(x, y, wrt,
  ...
  if(any(is.null(dim(eval.points)) || dim(eval.points)[2]==1)){
    ...
    if(!is.null(dim(eval.points)) && dim(eval.points)[2]==1){
      sampsize <- 10
      index <- apply(sapply(quantile(unlist(eval.points), seq(0,1,.1)), function(z) abs(z - unlist(eval.points))), 2, which.min)
