...
NNS.SSD <- function(x,y){
  ...
  LPM_y_sort=LPM(1,Combined_sort,y)
    ...
    ifelse (x.ssd.y==length(Combined) & min(x)>=min(y) & !identical(LPM_x_sort,LPM_y_sort),"X SSD Y",
            ifelse (y.ssd.x==length(Combined) & min(y)>=min(x) & !identical(LPM_x_sort,LPM_y_sort),"Y SSD X","NO SSD EXISTS"))
}
