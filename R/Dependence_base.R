...
NNS.dep.base <- function(x,
  ...
  if(!missing(y)) {
    ...
    if(any(length(unique(x)) < sqrt(length(x)) | length(unique(y)) < sqrt(length(y))  | is.na(sd(x)) | is.na(sd(y)) | sd(x)==0 | sd(y)==0 | length(x) < 20)){
      ...
      if(asym){
          disp <- min_part.df[,"nns_results" := sign(cor(x,abs(y)))*summary(lm(abs(y)~poly(x,min(5, floor(log(length(x),4)+2)),raw = TRUE)))$r.sq, by = quadrant]
      } else {
          disp <- min_part.df[,"nns_results" := sign(cor(x,y))*summary(lm(y~poly(x,min(5, floor(log(length(x),4)+2)),raw = TRUE)))$r.squared, by = quadrant]
      }
