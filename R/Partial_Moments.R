...
LPM <-  function(degree, target, variable){
    ...
    if(any(class(x)==c("tbl", "data.table"))) variable <- as.vector(unlist(variable))
...
Co.UPM <- function(degree.x, degree.y, x, y, target.x = mean(x), target.y = mean(y)){
  ...
  if(any(class(x)==c("tbl", "data.table"))) y <- as.vector(unlist(y))
...
PM.matrix <- function(LPM.degree, UPM.degree, target = NULL, variable, pop.adj=FALSE){
  ...
  if(any(class(x)==c("tbl", "data.table"))) variable <- as.data.frame(variable)
