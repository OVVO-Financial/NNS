...
The same workflow in Python uses the `nns` package (installed as `ovvo-nns`):
...
# Empirical CDF evaluated at 0 and 1
np.mean(x <= 0), np.mean(x <= 1)

# Degree-zero LPM reproduces the empirical CDF
lpm(0, 0.0, x), lpm(0, 1.0, x)

# Vectorized targets:
lpm(0, np.array([0.0, 1.0]), x)

# Full CDF trace: degree-zero LPM at every sorted observation
lpm(0, np.sort(x), x)
```
