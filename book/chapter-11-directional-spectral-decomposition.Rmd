...
## Quadrant Decomposition in Python

The entire workflow of this chapter is base linear algebra, so the Python translation needs only NumPy. The function below reproduces `quadrant_decomposition`, and the calls that follow reproduce the rank-one verification, the conditional-mean eigenvector recovery, the centroid contrasts, and the eigenvalue attribution.
...
def pop_cov(Z):
    Zc = Z - Z.mean(axis=0)
    return Zc.T @ Zc / Zc.shape[0]

def quadrant_decomposition(Z):
    n = Z.shape[0]
    mu = Z.mean(axis=0)
    mx, my = mu
    quadrants = {
    ...
    for qname, mask in quadrants.items():
        ...
        m_q = Z_q.mean(axis=0)
        u_q = m_q - mu
        Sigma_Q += p_q * np.outer(u_q, u_q)
        Sigma_W += p_q * pop_cov(Z_q)
        centroids[qname] = {"n": n_q, "p": p_q, "m": m_q, "u": u_q,
                            "lambda_rank1": p_q * float(u_q @ u_q)}
    return {"mu": mu, "Sigma": pop_cov(Z),
            "Sigma_Q": Sigma_Q, "Sigma_W": Sigma_W, "centroids": centroids}

# Positively dependent bivariate data
rng = np.random.default_rng(123)
n, rho = 10_000, 0.70
R = np.array([[1.0, rho], [rho, 1.0]])
Z = rng.standard_normal((n, 2)) @ np.linalg.cholesky(R).T

D = quadrant_decomposition(Z)
eigval_classical, V = np.linalg.eigh(D["Sigma"])
# eigh sorts ascending; reverse to match R's eigen ordering
eigval_classical, V = eigval_classical[::-1], V[:, ::-1]

# Rank-one primitive verification: B_q u_q = p_q ||u_q||^2 u_q,
# and the four primitives reconstruct Sigma_Q
Sigma_Q_from_Bq = np.zeros((2, 2))
for qname, c in D["centroids"].items():
    Bq = c["p"] * np.outer(c["u"], c["u"])
    np.testing.assert_allclose(Bq @ c["u"], c["lambda_rank1"] * c["u"])
    Sigma_Q_from_Bq += Bq
np.testing.assert_allclose(Sigma_Q_from_Bq, D["Sigma_Q"])

# Conditional-mean eigenvector recovery: {p_q, m_q} -> C -> CC^T -> eig
U = np.array([c["u"] for c in D["centroids"].values()])
P = np.array([c["p"] for c in D["centroids"].values()])
C = (np.sqrt(P)[:, None] * U).T
Sigma_Q_from_means = C @ C.T
eigval_Q, V_Q = np.linalg.eigh(Sigma_Q_from_means)
eigval_Q, V_Q = eigval_Q[::-1], V_Q[:, ::-1]

# Centroid contrast directions
unit = lambda v: v / np.linalg.norm(v)
v_concordant = unit(D["centroids"]["CUPM"]["m"] - D["centroids"]["CLPM"]["m"])
v_divergent  = unit(D["centroids"]["DLPM"]["m"] - D["centroids"]["DUPM"]["m"])
abs(v_concordant @ V_Q[:, 0])   # alignment with first Sigma_Q eigenvector
abs(v_divergent  @ V_Q[:, 1])   # alignment with second Sigma_Q eigenvector

# Eigenvalue attribution via the Rayleigh quotient:
# lambda_i = v_i' Sigma_Q v_i + v_i' Sigma_W v_i
for i in range(2):
    v = V[:, i]
    between = v @ D["Sigma_Q"] @ v
    within  = v @ D["Sigma_W"] @ v
    np.testing.assert_allclose(between + within, eigval_classical[i])

# Quadrant-level attribution of the between component: p_q (v' u_q)^2
for qname, c in D["centroids"].items():
    contribution_pc1 = c["p"] * float(V[:, 0] @ c["u"]) ** 2
```
