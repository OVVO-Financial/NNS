...
The directional co-partial moment matrices are obtained directly from `NNS::PM.matrix()`.
Population normalization \(1/n\) is used throughout to match the expectation formulas above, so `pop_adj = FALSE` is used.

For degree-one co-partial moments,
...
Sigma <- pm$clpm + pm$cupm - pm$dlpm - pm$dupm
...
max(abs(pm$cov.matrix - Sigma))
```

The `PM.matrix()` object reports the four directional co-partial moment matrices and the reconstructed covariance matrix. The concordant terms, `clpm` and `cupm`, enter positively, while the divergent terms, `dlpm` and `dupm`, enter negatively.

The recovered covariance matrix is then passed directly to `eigen()` for spectral decomposition. For the simulated positively dependent data, the leading eigenvector lies approximately along the concordant direction, while the second eigenvector lies along the opposing contrast direction.
...
Directional co-partial moment attribution is computed directly from the matrices returned by `NNS::PM.matrix`.
...
pm_attr_1 <- attribute_pm_lambda(V[, 1], pm)
pm_attr_2 <- attribute_pm_lambda(V[, 2], pm)

rbind(lambda_1 = pm_attr_1, lambda_2 = pm_attr_2)
eig_classical$values
```

For this positively dependent example the pattern is clear.
The leading eigenvalue is generated primarily by lower and upper concordant co-movement.
The second eigenvalue has a larger relative divergent contribution.
