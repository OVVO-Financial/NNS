...
This chapter develops a nonparametric approach to distribution comparison based on directional probability measures and effect-size interpretations rather than hypothesis-testing decisions. It then introduces the NNS ANOVA procedure, which operationalizes these ideas through the LPM-based continuous CDF, and concludes with stochastic dominance tests that extend the comparison to ordered preference relations.

---

This chapter is organized into four signposted blocks so readers can move from concepts to implementation: **Block I (Theory)** develops comparison logic and the continuous-CDF correction; **Block II (Estimation mechanics)** defines operational estimators including NNS ANOVA; **Block III (Diagnostics)** covers dominance curves and ordered-comparison diagnostics; **Block IV (Applied workflow)** provides examples and practical guidance.
...
This is also the bridge to Chapter 15: the degree-one quantile objects used for bias-corrected prediction intervals (`LPM.VaR(..., degree = 1, ...)`, `UPM.VaR(..., degree = 1, ...)`) are generated from the same degree-one lower/upper partial moment geometry used here for SSD and TSD diagnostics. Put differently, interval construction and dominance testing are not separate methods — they are two uses of the same directional probability representation.
...
### Example Dataset Workflow (Parts III–V)

For cross-part continuity, reuse the same dataset from Chapter \@ref(example-dataset) and define two groups from the predictor distribution, for example:

- Group A: observations with `x` in the bottom quartile.
- Group B: observations with `x` in the top quartile.

Then apply this chapter's comparison workflow in sequence:

1. Estimate directional dominance probability `P(A > B)` (or the reverse, depending on target variable orientation).
2. Compute directional probability effect-size summaries.
3. Run `NNS.ANOVA` with the continuous CDF option for full-distribution comparison.
4. Archive the effect-size outputs for later comparison with Chapter 18 predictive performance diagnostics.

Using the same data object across chapters keeps interpretation stable: dependence findings from Part III map directly to distribution-comparison in Part IV and then to estimator behavior in Part V.


### Summary

Classical distribution comparison relies heavily on hypothesis testing procedures that emphasize binary decisions.

The directional framework offers a complementary approach based on **probability comparisons and directional partial moments**.

Key ideas developed in this chapter include:

- Distribution comparison through probabilities such as $P(X>Y)$.
- Effect-size interpretation using directional exceedance probabilities such as $P(X' > Y')$.
- Benchmark-based comparisons using divergent co-partial moments.
- Directional dominance curves revealing where distributions differ.
- **The discrete–continuous CDF distinction**: the empirical CDF ($d=0$) is systematically biased at the mean for any finite sample; the degree-one partial moment ratio ($d=1$) eliminates this bias exactly, with $F_1(\bar{x}; X) = 0.5$ holding without exception.
- NNS ANOVA: CDF-based distributional comparison using the degree-one continuous CDF, producing an agreement certainty statistic $\text{Certainty}_{\text{ANOVA}} \in [0,1]$ (with $1$ indicating greatest agreement) alongside effect sizes and robust confidence intervals. The mean-target property of Section 15.1.8 is what makes this procedure distribution-free and bias-free.
- Stochastic dominance at first, second, and third order, expressed as partial moment inequalities and testable through empirical estimates.
- Efficient sets identifying distributions not dominated by any alternative.
- Dominance clustering for hierarchical grouping of distributions by preference order.
- Fully nonparametric inference based on empirical probabilities and partial moments throughout.

These tools provide a flexible and interpretable method for comparing distributions without parametric assumptions.

The next chapter develops **prediction intervals**, showing how the degree-one partial moment CDF and its quantile functions extend the bias-elimination framework to interval estimation for future observations.

## References

- Viole, F., & Nawrocki, D. (2016). LPM density functions for the computation of the SD efficient set. *Journal of Mathematical Finance*, **6**(1), 105–126. https://doi.org/10.4236/jmf.2016.61012
