...
Chapter 18 established the recursive mean-split estimator as a **partition-based nonparametric regression method** and showed that, under standard regularity conditions, it is consistent for the true conditional mean function.
...
Chapter 18 established that consistency requires two conditions:
...
The bias–variance decomposition from Chapter 18 then becomes
...
The shrinking-diameter condition from Chapter 18 is simply the shrinking-bandwidth condition in disguise.
...
Chapter 18 then showed that recursive mean splitting uses benchmark-relative geometry to define estimation regions and produces a consistent estimator.
...
## Vignette Figure: Order-Driven Bandwidth Contraction

![Figure 19.1. `NNS.reg` across increasing orders (including `order = \"max\"`), showing shrinking effective cell diameter and increasingly local fit.](../vignettes/images/overview_reg.png)
...
- The consistency conditions from Chapter 18 become the direct analogues of shrinking-bandwidth conditions in kernel regression.
