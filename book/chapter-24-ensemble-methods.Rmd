...
Both ensemble interfaces are exposed in Python as `nns_boost` and `nns_stack`, with the same controls in snake_case (`learner_trials`, `cv_size`, `dim_red_method`, `obj_fn`, `objective`, ...). A minimal numeric-regression workflow:
...
# Feature-screening boosted ensemble
boosted = nns_boost(X_train, y_train, X_test,
                    learner_trials=100, status=False)
boosted["results"]             # final predictions
boosted["feature.weights"]     # normalized feature-survival frequencies

# Stacked ensemble across method 1 (regression-point kNN) and
# method 2 (dimension-reduction regression), with cross-validated k
stacked = nns_stack(X_train, y_train, X_test,
                    method=(1, 2), dim_red_method="cor", folds=5, status=False)
