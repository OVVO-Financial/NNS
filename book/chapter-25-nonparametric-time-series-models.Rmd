...
```python
import numpy as np
from nns import nns_seas, nns_arma_optim

# Univariate nonlinear ARMA
rng = np.random.default_rng()
z = np.sin(np.arange(1, 481) / 8) + rng.normal(scale=0.35, size=480)
z = (z - z.mean()) / z.std(ddof=1)

# Seasonality detection
seasonal_period = nns_seas(z, plot=False)
seasonal_period["all.periods"]

# Validate seasonal periods and forecast
nns_arma_optim(z, h=48, seasonal_factor=seasonal_period["periods"])
```


<center>
![Figure 25.1. `NNS.ARMA(..., h = 45, seasonal.factor = c(12,24,36))` forecast output with fitted trajectory and uncertainty structure.](images/ch25_uni_ts.png)
</center>
