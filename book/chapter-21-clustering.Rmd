...
The same comparison in Python, with `scikit-learn` supplying the k-means baseline:

```python
import numpy as np
from sklearn.cluster import KMeans
from nns import nns_part
from nns.plotting import plot_nns_part

# Clustered dataset
g = 6
rng = np.random.default_rng(g)
n = 100
x = np.concatenate([rng.normal(rng.uniform() * i**2, 1, n // g) for i in range(1, g + 1)])
y = np.concatenate([rng.normal(rng.uniform() * i**2, 1, n // g) for i in range(1, g + 1)])
d = np.column_stack((x, y))

km = KMeans(n_clusters=3, n_init="auto").fit(d)
km.labels_, km.cluster_centers_

part = nns_part(x, y, order=3, obs_req=0)
plot_nns_part(part)   # Voronoi view of the directional partition
```

<center>
![Figure 21.1. k-means (top) with analyst-specified $K=3$ versus NNS.part (bottom) with data-determined clusters. The directional partition discovers the natural group structure without a prespecified count; k-means forces all observations into exactly three spherical regions regardless of whether three is the right number.](images/ch21_kmeans_comp.png)
</center>
