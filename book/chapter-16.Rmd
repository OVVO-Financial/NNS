...
Both functions accept a `degree` argument. Setting `degree = 0` uses the discrete empirical CDF; setting `degree = 1` uses the continuous area-based probability established in Section 15.1.8 — eliminating finite-sample bias from the interval bounds.

For a prediction interval with coverage $1 - \alpha$:

```r
# 95% prediction interval using continuous (degree = 1) quantiles
lower <- LPM.VaR(0.025, degree = 1, variable = x)
upper <- UPM.VaR(0.025, degree = 1, variable = x)
```
...
LPM.VaR(0.025, degree = 0, boot_out$t)
## [1] 0.4688333

UPM.VaR(0.025, degree = 0, boot_out$t)
## [1] 0.9632222
```
...
LPM.VaR(0.025, degree = 1, boot_out$t)
## [1] 0.5612749

UPM.VaR(0.025, degree = 1, boot_out$t)
## [1] 0.8263255
```
...
LPM.VaR(0.05, degree = 0, x)
## [1] -2
UPM.VaR(0.05, degree = 0, x)
## [1] 10

# Degree 1: continuous area-based quantiles
LPM.VaR(0.05, degree = 1, x)
## [1] -1.65
UPM.VaR(0.05, degree = 1, x)
## [1] 9.15
```
...
**Interpretability.**  
Intervals correspond directly to probability statements about future observations, rooted in the degree-zero and degree-one partial moment representation of the distribution.
...
These ideas complete the core framework for **nonparametric statistical inference using directional statistics**. The next chapter extends the methodology to **synthetic data generation and bootstrap methods**, enabling uncertainty quantification and simulation in complex nonlinear systems.
