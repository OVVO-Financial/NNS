...
`NNS.reg` also provides a dimension reduction regression by including a parameter `NNS.reg(x,y,dim.red=TRUE,...)`.  Reducing all regressors to a single dimension using the returned equation `$equation`.
```{r nonlinear class}
NNS.reg(iris[,1:4],iris[,5],dim.red=TRUE)$equation
```

####Threshold
`NNS.reg(x,y,dim.red=TRUE,threshold=...)` offers a method of reducing regressors further by controlling the absolute value of required correlation.
