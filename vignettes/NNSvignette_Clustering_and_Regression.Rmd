...
# Cross-Validation
The `NNS.stack()` routine cross-validates for a given objective function the `n.best` parameter in the multivariate `NNS.reg` function as well as the `threshold` parameter in the dimension reduction `NNS.reg` version.  `NNS.stack` can be used for classification or continuous dependent variables.
