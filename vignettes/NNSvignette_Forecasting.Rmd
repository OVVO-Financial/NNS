...
There is a benefit to using a nonlinear regression as our RMSE has been lowered.  We can also test if using both linear and nonlinear estimates combined result in a lower RMSE using `method = "both"`.
...
Using `method = "both"` did not lower our RMSE.  There are far fewer parameters to test with NNS than traditional methods and the relative simplicity of the method ensures robustness.
...
## `$ensemble == TRUE`
If the results of `NNS.ARMA.optim` indicate `$ensemble == TRUE`, then it has determined that an ensemble forecast has outperformed in the objective function used.  In order to create the ensemble forecast, we need to generate the alternative predictions.

```{r ensemble}
nns.optims <- NNS.ARMA.optim(AirPassengers[1:132], training.set = 120,
seasonal.factor = seq(12, 24, 6))

## If {$ensemble == TRUE} then take the mean of both forecasts, using {best.periods = length(seasonal.factor)}in the {NNS.ARMA} function.

## Using optimized paramaters from {NNS.ARMA.optim}
nns.estimates.1 <- NNS.ARMA(AirPassengers, h = 12, training.set = 132,
seasonal.factor = nns.optims$periods, method = nns.optims$method, plot = FALSE)

## Using all {seasonal.factor} considered in {NNS.ARMA.optim}
nns.estimates.2 <- NNS.ARMA(AirPassengers, h = 12, training.set = 132,
seasonal.factor = FALSE, best.periods = length(seq(12,24,6)), method = nns.optims$method, plot = FALSE)
