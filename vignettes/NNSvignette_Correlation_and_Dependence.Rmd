...
We can generate new data from the dependence structure of our `original data` via the following steps:

* **Determine the dependence structure:**

This is accomplished using `LPM.ratio(1, x, x)` for continuous variables, and `LPM.ratio(0, x, x)` for discrete variables.

* **Generate or supply the new data:**
...
* **Apply our dependence structure to our `new data`:**

We then utilize `LPM.VaR(...)` to ascertain `new data` values corresponding to `original data` position mappings, and return a matrix with the same dimensions as `original.data`.
...
```{r comparison}
NNS.dep.hd(original.data)
NNS.dep.hd(new.dep.data)
```
