...
Calculating the probability for every value in $x$, we can plot both methods visualizing their identical results.  `ecdf` function in black and **`LPM.ratio`** in red.
...
We will generate 1 replicate of $x$ for each value of a sequence of $\rho$ values, and then plot the results compared to our original $x$ (black line).
...
More replicates and ensembles thereof can be generated for any number of $\rho$ values.  Please see the full **`NNS.meboot`** argument documentation.
...
We will generate 10 replicates of $\rho = 0.95$ to our `original.data` and note the multivariate dependence among our `new.boot.dep.data`.
